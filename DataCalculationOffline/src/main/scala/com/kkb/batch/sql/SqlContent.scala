...
object SqlContent {
      ...
      //todo:2、国内查询转化率(查询--->旅客信息)      结果存储mysql的表: nh_domestic_inter_conversion_rate
        val natinalRateSQL=
          """
            ...
            |    0,
            |    0,
            |    round(sum(case when requestType = '2' then 1 else 0 end) / cast(sum(case when requestType = '1' then 1 else 0 end) as double),2),
            |   max(visitTime)
            |from processTag
    ...
    val natinalTravellerRateSQL=
      """
        ...
        |    1,
        |    0,
        |   round(sum(case when requestType = '3' then 1 else 0 end) / cast(sum(case when requestType = '2' then 1 else 0 end) as double),2),
        |   max(visitTime)
        |from processTag
        |where flightType = 'National'
      """.stripMargin
    ...
    val internatinalTravellerRateSQL=
      ...
      val spiderQueryTravellerRateSQL=
        """
          ...
          |    0,
          |    1,
          |   sum(case when requestType = '2' then 1 else 0 end)/ cast(sum(case when requestType = '1' then 1 else 0 end) as double),
          |   max(visitTime)
          |from processTag
          |where isSpider = '1'
        """.stripMargin
      ...
      val spiderTravellerPayRateSQL=
        """
          ...
          |    1,
          |    1,
          |   sum(case when requestType = '3' then 1 else 0 end) / cast(sum(case when requestType = '2' then 1 else 0 end) as double),
          |   max(visitTime)
          |from processTag
          |where isSpider = '1'
        """.stripMargin
      ...
      val spiderBookQueryRateSQL=
        """
          ...
          |    uniqueFlag(),
          |    2,
          |   sum(case when requestType = '1' then 1 else 0 end)/ cast(sum(case when requestType = '0' then 1 else 0 end) as double),
          |   max(visitTime)
          |from processTag
          |where isSpider = '1'
        """.stripMargin
